...
(defn upsert-value
  ...
  (assert (and secret-name (type secret-value))
          (format "Secret %s not found, looked up as %s. Check the workflow/job/step `env` as well as the known environment variables in config."
                  secret-name (type secret-value)))
